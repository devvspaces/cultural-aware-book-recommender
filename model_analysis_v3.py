...
from cultural_aware_fm_v3 import CulturallyAwareFMv3
...
def evaluate_ranking_and_diversity(df_test, df_train, books, model_context, svd_model, fm_v1, fm_v2, fm_v3,
    ...
    metrics = {
        ...
        'fm_v3': {'prec': [], 'rec': [], 'ild': [], 'novelty': [], 'rec_books': set()}
    }
    ...
    for user_idx in test_users:
        ...
        for b_idx in candidate_indices:
            ...
            f3_score = fm_v3.predict(
                user_id=None if is_cold_start else user_idx,
                book_idx=b_idx,
                user_hof=orig_user_hof,
                book_hof=book_vectors[b_idx]
            )
            fm_v3_scores.append((b_idx, f3_score))
        ...
        rec_fm_v3 = [b for b, s in sorted(fm_v3_scores, key=lambda x: x[1], reverse=True)[:k]]
        
        for name, rec_list in [('content', rec_content), ('svd', rec_svd), ('fm_v1', rec_fm_v1), ('fm_v2', rec_fm_v2), ('fm_v3', rec_fm_v3)]:
    ...
    summary = {}
    for name in ['content', 'svd', 'fm_v1', 'fm_v2', 'fm_v3']:
        summary[name] = {
...
def main():
    ...
    # C. Culturally Aware FM (v3 Multi-Hot Genres + Explicit Distance)
    print("\n--- Training FM v3 (Multi-Hot Genres + Cultural Distances) ---")
    fm_v3 = CulturallyAwareFMv3(num_factors=10, learning_rate=0.01, regularization=0.03, epochs=8, min_genre_count=5)
    fm_v3.load_hofstede_csv(hofstede_path)
    fm_v3.fit(df_train, books, book_csv_to_idx, user_profiles, book_vectors)
    ...
    preds = {'content': [], 'svd': [], 'fm_v1': [], 'fm_v2': [], 'fm_v3': []}
    actuals = []
    ...
    for idx, row in df_test.iterrows():
        ...
        preds['fm_v3'].append(fm_v3.predict(u_idx, b_idx, u_hof, b_hof))
    ...
    for idx, row in df_cold.iterrows():
        ...
        cold_preds['fm_v3'].append(fm_v3.predict(None, b_idx, fm_v3.global_average_hofstede, b_hof))
    ...
    rmse_cf3, mae_cf3 = compute_metrics(cold_actuals, cold_preds['fm_v3'])
    ...
    normal_ranking = evaluate_ranking_and_diversity(
        df_test, df_train, books, model_context, svd_model, fm_v1, fm_v2, fm_v3,
        user_profiles, book_vectors, user_train_history, is_cold_start=False
    )
    
    cold_ranking = evaluate_ranking_and_diversity(
        df_cold, df_train, books, model_context, svd_model, fm_v1, fm_v2, fm_v3,
        user_profiles, book_vectors, user_train_history, is_cold_start=True
    )
    
    print("\n" + "="*125)
    print("                     GOODREADS MODEL EVALUATION COMPARISON REPORT (V1 vs V2 vs V3)")
    print("="*125)
    
    table_data = [
        ["Active User RMSE", f"{rmse_c:.4f}", f"{rmse_s:.4f}", f"{rmse_f1:.4f}", f"{rmse_f2:.4f}", f"{rmse_f3:.4f}"],
        ["Active User MAE", f"{mae_c:.4f}", f"{mae_s:.4f}", f"{mae_f1:.4f}", f"{mae_f2:.4f}", f"{mae_f3:.4f}"],
        ["Active Precision@5", f"{normal_ranking['content']['precision']:.4f}", f"{normal_ranking['svd']['precision']:.4f}", f"{normal_ranking['fm_v1']['precision']:.4f}", f"{normal_ranking['fm_v2']['precision']:.4f}", f"{normal_ranking['fm_v3']['precision']:.4f}"],
        ["Active Recall@5", f"{normal_ranking['content']['recall']:.4f}", f"{normal_ranking['svd']['recall']:.4f}", f"{normal_ranking['fm_v1']['recall']:.4f}", f"{normal_ranking['fm_v2']['recall']:.4f}", f"{normal_ranking['fm_v3']['recall']:.4f}"],
        ["Active Diversity (ILD)", f"{normal_ranking['content']['ild']:.4f}", f"{normal_ranking['svd']['ild']:.4f}", f"{normal_ranking['fm_v1']['ild']:.4f}", f"{normal_ranking['fm_v2']['ild']:.4f}", f"{normal_ranking['fm_v3']['ild']:.4f}"],
        ["Active Novelty", f"{normal_ranking['content']['novelty']:.4f}", f"{normal_ranking['svd']['novelty']:.4f}", f"{normal_ranking['fm_v1']['novelty']:.4f}", f"{normal_ranking['fm_v2']['novelty']:.4f}", f"{normal_ranking['fm_v3']['novelty']:.4f}"],
        ["Active Coverage", f"{normal_ranking['content']['coverage']*100:.1f}%", f"{normal_ranking['svd']['coverage']*100:.1f}%", f"{normal_ranking['fm_v1']['coverage']*100:.1f}%", f"{normal_ranking['fm_v2']['coverage']*100:.1f}%", f"{normal_ranking['fm_v3']['coverage']*100:.1f}%"],
        ["-"*24, "-"*10, "-"*10, "-"*10, "-"*10, "-"*10],
        ["Cold-Start RMSE", f"{rmse_cc:.4f}", f"{rmse_cs:.4f}", f"{rmse_cf1:.4f}", f"{rmse_cf2:.4f}", f"{rmse_cf3:.4f}"],
        ["Cold-Start MAE", f"{mae_cc:.4f}", f"{mae_cs:.4f}", f"{mae_cf1:.4f}", f"{mae_cf2:.4f}", f"{mae_cf3:.4f}"],
        ["Cold Precision@5", f"{cold_ranking['content']['precision']:.4f}", f"{cold_ranking['svd']['precision']:.4f}", f"{cold_ranking['fm_v1']['precision']:.4f}", f"{cold_ranking['fm_v2']['precision']:.4f}", f"{cold_ranking['fm_v3']['precision']:.4f}"],
        ["Cold Recall@5", f"{cold_ranking['content']['recall']:.4f}", f"{cold_ranking['svd']['recall']:.4f}", f"{cold_ranking['fm_v1']['recall']:.4f}", f"{cold_ranking['fm_v2']['recall']:.4f}", f"{cold_ranking['fm_v3']['recall']:.4f}"],
        ["Cold Diversity (ILD)", f"{cold_ranking['content']['ild']:.4f}", f"{cold_ranking['svd']['ild']:.4f}", f"{cold_ranking['fm_v1']['ild']:.4f}", f"{cold_ranking['fm_v2']['ild']:.4f}", f"{cold_ranking['fm_v3']['ild']:.4f}"],
        ["Cold Novelty", f"{cold_ranking['content']['novelty']:.4f}", f"{cold_ranking['svd']['novelty']:.4f}", f"{cold_ranking['fm_v1']['novelty']:.4f}", f"{cold_ranking['fm_v2']['novelty']:.4f}", f"{cold_ranking['fm_v3']['novelty']:.4f}"],
        ["Cold Coverage", f"{cold_ranking['content']['coverage']*100:.1f}%", f"{cold_ranking['svd']['coverage']*100:.1f}%", f"{cold_ranking['fm_v1']['coverage']*100:.1f}%", f"{cold_ranking['fm_v2']['coverage']*100:.1f}%", f"{cold_ranking['fm_v3']['coverage']*100:.1f}%"],
    ]
    
    headers = ["Evaluation Metric", "Content-Based", "Surprise SVD++", "Cultural FM v1", "Cultural FM v2", "Cultural FM v3"]
